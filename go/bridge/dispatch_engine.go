...
func dispatchEngine(method string, payload []byte) ([]byte, error) {
	...
	case "ComposeWithAI":
		...
		if req.ChangeTone != "" {
			// gotd >= v0.152: the old change_tone string flag became
			// Tone InputAiComposeToneClass. The "default tone by name"
			// constructor preserves the old semantics.
			aiReq.SetTone(&tg.InputAiComposeToneDefault{Tone: req.ChangeTone})
		}
