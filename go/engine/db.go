...
func migrateV27(tx *sql.Tx) error {
	if !columnExists(tx, "chats", "can_post") {
		if _, err := tx.Exec(`ALTER TABLE chats ADD COLUMN can_post INTEGER NOT NULL DEFAULT 0`); err != nil {
			return err
		}
	}
	return nil
...
func migrateV28(tx *sql.Tx) error {
	if !columnExists(tx, "messages", "is_deleted") {
		if _, err := tx.Exec(`ALTER TABLE messages ADD COLUMN is_deleted INTEGER NOT NULL DEFAULT 0`); err != nil {
			return err
		}
	}
	if !columnExists(tx, "messages", "deleted_at") {
		if _, err := tx.Exec(`ALTER TABLE messages ADD COLUMN deleted_at INTEGER`); err != nil {
			return err
		}
	}
	return nil
...
func migrateV33(tx *sql.Tx) error {
	if !columnExists(tx, "chats", "is_admin") {
		if _, err := tx.Exec(`ALTER TABLE chats ADD COLUMN is_admin INTEGER NOT NULL DEFAULT 0`); err != nil {
			return err
		}
	}
	return nil
...
func migrateV34(tx *sql.Tx) error {
	if !columnExists(tx, "users", "last_seen_kind") {
		if _, err := tx.Exec(`ALTER TABLE users ADD COLUMN last_seen_kind TEXT`); err != nil {
			return err
		}
	}
	if !columnExists(tx, "users", "has_personal_photo") {
		if _, err := tx.Exec(`ALTER TABLE users ADD COLUMN has_personal_photo INTEGER NOT NULL DEFAULT 0`); err != nil {
			return err
		}
	}
	return nil
...
func migrateV37(tx *sql.Tx) error {
	if !columnExists(tx, "users", "note") {
		if _, err := tx.Exec(`ALTER TABLE users ADD COLUMN note TEXT`); err != nil {
			return err
		}
	}
...
func migrateV40(tx *sql.Tx) error {
	if columnExists(tx, "messages", "forward_from_id") {
		return nil
	}
