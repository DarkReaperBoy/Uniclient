...
- [ ] [MAJOR] Whole-peer report uses the legacy `account.reportPeer` + static reason picker, which is NOT AyuGram's actual peer-report UX (functional, but a ground-truth fidelity deviation — logged during the commit-329d3812 Stage-2 verify). AyuGram `ShowReportMessageBox` (`report_messages_box.cpp`, called with empty ids from `info_profile_actions.cpp:2913` / `info_profile_top_bar.cpp:998` / `window_peer_menu.cpp:993`) calls `messages.report` with empty ids and, on `MESSAGE_ID_REQUIRED`, shows a MESSAGE CHOOSER (`SessionController::showChooseReportMessages`) then re-issues `messages.report` with the picked ids — it never calls `account.reportPeer` (the static `ReportReasonBox` path is photo-only now → `account.reportProfilePhoto`, `api_report.cpp:44`). The current fix (`confirm_box.dart` `_showPeerReportFlow`) routes ALL empty-msgId reports to `account.reportPeer`, bypassing the existing dynamic flow even for peers that would return server-driven options. Future: on `MESSAGE_ID_REQUIRED` inside `showDynamicReportFlow`, present a recent-messages chooser and retry `messages.report` with the chosen ids, matching AyuGram. NOTE: a SUCCESSFUL `account.reportPeer` submit was not live-verified (only the engine→Go→server round-trip + failure-toast path, against a `PEER_ID_INVALID`-broken test peer; a real successful submit was deliberately not sent to avoid a real report against a third-party community) — verify a true success against a safe peer when revisiting. — `confirm_box.dart` `_showPeerReportFlow`/`_PeerReportReasonBox` ← `AyuGram report_messages_box.cpp` + `api_report.cpp`
