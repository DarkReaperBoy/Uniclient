...
- [ ] [MAJOR] `clearForChat/clearForItem/clearForTopic/clearForSublist/clearForAccount` filter `_queue` only AFTER the dismiss loop, but `dismiss()` promotes the next queued item (of ANY chat) into `_active` mid-loop — so a queued notification for the very chat/account being cleared gets promoted, shown, and then SURVIVES the post-loop queue filter (it is no longer in `_queue`). AyuGram erases the queue FIRST, then unlinks active, then `showNextFromQueue()`, so the promoted item can never be one being cleared. Repro w/ defaults (maxVisible 3): `_active=[X1,X2,X3]`, `_queue=[X4,Y1]`; `clearForChat(X)` leaves X4 visible. Same inverted order in all five methods: clearForChat loop `notification_manager_default.dart:226-228` then filter `:229-230`; clearForItem `:242-244`/`:245-248`; clearForTopic `:261-263`/`:264-268`; clearForSublist `:281-283`/`:284-288`; clearForAccount `:297-299`/`:300`; promotion in `dismiss` `notification_manager_default.dart:148-150` ← `AyuGramDesktop/Telegram/SourceFiles/window/notifications_manager_default.cpp:425-439` (and doClearFromTopic `:389-405`, doClearFromSublist `:407-423`, doClearFromSession `:441-455`, doClearFromItem `:457-472` — all erase queue before unlink+showNextFromQueue)

- [ ] [MAJOR] The five engine-driven clears remove popups INSTANTLY (`dismiss` → `onDismiss` → `_onDismissExternal`, which deletes the popup with no animation) instead of AyuGram's `hideFast()` 150ms fade. This is also the tap path: tapping a notification calls `clearForChat` (`notification_popup.dart:421`), so on every tap the chat's notifications vanish abruptly instead of fading as the chat opens. Inconsistent with this same file's `clearAll`→`hideAll`, which correctly drives the fast-fade channel (`onStartHidingFast`/`onStartHidingHideAllFast`). Clear path: `dismiss` calls `onDismiss` `notification_manager_default.dart:146` (used by clearForChat `:227` et al.); instant removal at `notification_popup.dart:323-331`; correct fast-fade pattern at `notification_manager_default.dart:205-208` ← `AyuGramDesktop/Telegram/SourceFiles/window/notifications_manager_default.cpp:434` (`doClearFromHistory`→`unlinkHistory`→`hideFast()` defined `:1202`, `:570-572`; contrast `doClearAll` `:375-381` also using `unlinkHistory`/`hideFast`)
...
- [ ] [MAJOR] Self-chat / Replies-chat userpic uses colored initials instead of the dedicated glyph. AyuGram's `GenerateUserpic` routes `peer->isSelf()` → `EmptyUserpic::GenerateSavedMessages` (the bookmark icon) and `peer->isRepliesChat()` → `EmptyUserpic::GenerateRepliesMessages`, falling through to the initials userpic only for ordinary peers. The Dart has no such branch: an avatar-less self chat (fired scheduled reminders / Saved Messages, where `data.isSelf` is available) renders the title's initials (e.g. the "📅 Reminder" title → a broken/garbage glyph) instead of the Saved Messages bookmark. — `notification_manager_native.dart:657-665` (DBus branch) & `:879-886` (portal branch) / `_generatePlaceholderUserpic:294-379` ← `AyuGram/Telegram/SourceFiles/window/notifications_utilities.cpp:26-32`

- [ ] [MAJOR] Placeholder initials logic (`_getInitials`) does not match `EmptyUserpic::fillString` and breaks on emoji / non-BMP names. AyuGram explicitly **skips emoji** (`Ui::Emoji::Find`) and surrogate-pair code points when scanning for the first/second initial, and prefers a second letter after a space *or* hyphen. The Dart takes `words[0][0]` / `words[1][0]` — the first **UTF-16 code unit** of each word, with no emoji skipping and no surrogate handling. A peer/title beginning with an emoji or non-BMP char (e.g. "🔥Squad", or the "📅 " scheduled-message prefix) yields a lone high-surrogate that `img.drawString` cannot render → a blank/garbage circle, where AyuGram shows the first real letter ("S"). It also derives initials from the fully-composed notification title (which may carry the 📅 prefix, " ➜ account" suffix, or "Topic (Group)" decoration) rather than the peer name AyuGram feeds to `EmptyUserpic`. — `notification_manager_native.dart:284-292` (used at `:350`) ← `AyuGram/Telegram/SourceFiles/ui/empty_userpic.cpp:613-672`

- [ ] [MAJOR] Placeholder gradient is keyed by `title.hashCode`, not the peer's color index, so it does not match the color Telegram/AyuGram shows for the peer. AyuGram's `EmptyUserpic` color comes from `peer->colorIndex()` (base id-driven: `DecideColorIndex(id) = id % kSimpleColorIndexCount`). The Dart hashes the display title (`gradients[paletteMap[title.hashCode.abs() % 7]]`); the in-code justification ("only has a name, not the peer id") is incorrect — `data.chatId`/`data.senderId` (the peer id) are present in `NotificationData` and could drive `id % 7` to match. Result: avatar-less notifications get a different background color than the rest of the UI shows for that chat. — `notification_manager_native.dart:322-323` ← `AyuGram/Telegram/SourceFiles/ui/chat/chat_style.cpp:1198-1199` & `ui/empty_userpic.cpp:282-294`
...
- [ ] [CRITICAL] **Persisted proxy is not even re-synced to the engine at startup.** `initialize()` deliberately re-syncs ghost (`_syncGhostToEngine()`) and anti-recall (`_syncAntiRecallSettings()`) after loading prefs, but never calls `_syncProxyToEngine()`. Since the engine holds proxy in memory only and never loads it from disk, even if the engine *did* consume the fields (see above), a saved proxy would still be dropped on every restart until the user manually re-toggles a proxy setting. — `app_state.dart:3204,3287-3289` (initialize syncs ghost/antiRecall, omits proxy) / `app_state.dart:2605-2663` (`_syncProxyToEngine` only ever called from setters) ← `go/engine/engine.go:270-286` (no disk load) vs AyuGram `core/core_settings_proxy.h:73-74` (`serialize`/`setFromSerialized` — proxy persisted in Core::Settings and applied at startup).

- [ ] [MAJOR] **Engine settings pushed via `callGeneric` are never re-pushed at startup, so they silently revert to engine defaults after a restart** (unlike ghost/anti-recall which ARE re-synced). Concrete consumed example: per-source auto-download limits. `setAutoDownloadSettings` pushes `SetAutoDownload`, the engine stores it in-memory and `ShouldAutoDownload` actually reads it — but `initialize()` never re-pushes the loaded `_autoDownloadSettings`, so after restart the user's custom limits are gone and the engine falls back to defaults. Same pattern affects `SetLocalStorageLimits` (media/time eviction limits) and `SetPowerSaving`/`SetNotificationVolume`. — `app_state.dart:2694-2702` (`setAutoDownloadSettings`) + `app_state.dart:3204` (initialize omits re-push) ← `go/engine/engine.go:289-297` (in-memory only) vs AyuGram `data/data_auto_download.h` (`Full::setFromSerialized` — persisted in Core::Settings, applied at startup).

- [ ] [MAJOR] **`maxAccountLimit` uses the wrong formula.** AyuGram's `Domain::maxAccounts()` returns `min(premiumCount + 100, 200)` — each premium account raises the cap by exactly 1. The Dart returns 200 if *any* account is premium else 100, so with a single premium account it permits 200 accounts where AyuGram permits 101 (and 105 for 5 premium, etc.). — `app_state.dart:713-715` ← `main/main_domain.cpp:503-510`.

- [ ] [MAJOR] **Per-account ghost toggles push the GLOBAL engine config even in per-account mode.** `setSendReadMessages` / `setSendReadStories` / `setSendOnlinePackets` / `setSendUploadProgress` / `setSendOfflinePacketAfterOnline` mutate the active account's resolved `GhostModeAccountSettings` (per-account-correct) but then call `_engine.updateConfig(...)` with **no `accountId`** (global scope). This contradicts the sibling `setMarkReadAfterAction`/`setUseScheduledMessages` (which explicitly route through the per-account-aware `_syncGhostToEngine()`, and even carry a comment explaining exactly this) and `_syncGhostToEngine`'s per-account path (`updateConfig(accountId: acc.id, ...)`). In "individual settings for each account" mode the drawer LRead toggle therefore writes the wrong scope and can be overridden by the now-stale per-account override. AyuGram resolves ghost per session/userId and applies a per-account change only to that account. — `app_state.dart:1489-1532` (vs the correct pattern at `app_state.dart:1534-1554`) ← `ayu/ayu_settings.cpp:437-448` (`ghost(userId)` per-account resolution) + `ayu_settings.cpp:70-74` (per-account setter, no global push).

- [ ] [MAJOR] **Load path only partially replicates AyuGram's `validate()`.** AyuGram clamps every out-of-range persisted value on load: `messageBubbleRadius`→[0,16], `avatarCorners`→[0,kMaxAvatarCorners], all `ContextMenuVisibility`/`showPeerId`/`channelBottomButton` enums→[0,2], `embeddedThemeType`→{-1,0..3}. The Dart `_loadWindowPrefs` clamps only `recentStickersCount`, `wideMultiplier` and `translationProvider`; `_bubbleRadius`, the primary `_avatarCorners` key, the seven context-menu visibilities, `channelBottomButton`, `showPeerId` and `embeddedThemeType` are read raw. A corrupt / hand-edited / version-downgraded prefs value is then used as-is (e.g. a negative bubble radius or avatar-corner value flows straight into rendering) until the user next changes that setting. — `app_state.dart:3873` (bubbleRadius, no clamp), `app_state.dart:3877-3878` (avatarCorners direct key unclamped — only the legacy-migration branch clamps), `app_state.dart:3985-3991` (context-menu enums), `app_state.dart:4046,4018` (channelBottomButton, showPeerId) ← `ayu/ayu_settings.cpp:481-533` (`validate()` repairs all ranges/enums on load and re-saves).
...
- [ ] [MAJOR] `togglePlayback()` never clears `_resumeAfterCall`, so a manual play→pause **during a call** leaves the call-resume flag set and playback auto-resumes against the user's last action when the call ends. AyuGram clears `resumeOnCallEnd = false` on every manual `play()` / `playPause()` so an in-call manual interaction cancels the pending auto-resume. (Edge case, but a concrete state deviation; fix: set `_resumeAfterCall = false` in `togglePlayback`.) — `audio_service.dart:166-173` ← `Telegram/SourceFiles/media/player/media_player_instance.cpp:809` & `:1085`

- [ ] [MAJOR] `AudioRepeatMode.all` is inert — the completion auto-advance only special-cases `.one` and routes both `.all` and `.none` to the same plain `next()` (no playlist wrap), and `setRepeatMode` does not preload the boundary slice. AyuGram's `moveInPlaylist` applies modulo wrap-around for `RepeatMode::All` and `repeatChanges` fires `refreshPlaylist` to load the wrap slice, so repeat-all loops the playlist back at the ends; here repeat-all behaves identically to repeat-none. (Wrap must be added to the advance path / `moveAudioInPlaylist` callback.) — `audio_service.dart:328-339` & `:194-198` ← `Telegram/SourceFiles/media/player/media_player_instance.cpp:611-630` & `:169-175`

- [ ] [MAJOR] Shuffle / `OrderMode` is entirely unimplemented in the service. AyuGram's `Instance` (the class this file ports) carries a full shuffle subsystem — `OrderMode` (Default/Reverse/Shuffle), `ShuffleData`, `validateShuffleData`/`setupShuffleData`, and a shuffle branch in `moveInPlaylist` — driven by the player's order-toggle button. `audio_service.dart` has no order/shuffle state at all (`enum AudioRepeatMode { none, one, all }` only), so shuffled / reversed playback cannot occur end-to-end. — `audio_service.dart:10` (and whole file: no `OrderMode`) ← `Telegram/SourceFiles/media/player/media_player_instance.cpp:177-185`, `:565-607`, `:926-1068`

- [ ] [MAJOR] Playback-speed, repeat-mode and autoplay-next are user-unreachable, so this service's `setPlaybackSpeeds`/`setRepeatMode`/`setAutoplayNextDisabled` are permanently fed the defaults (1.0× / none / autoplay-on). They are wired only to `AppState` (synced once in `main.dart:359-368`), but the backing `AppState` setters (`setPlayerRepeatMode`/`setVoicePlaybackSpeed`/`setAudioPlaybackSpeed`/`setDisableAutoplayNext`) have **zero callers** — there is no now-playing player widget and no inline voice-speed toggle. AyuGram exposes these through `media_player_widget`'s repeat toggle (cycle None→One→All) and order toggle, plus the per-message speed control. (Fix lives in the player/settings UI, but it makes every speed/repeat/autoplay branch in this file dead in practice.) — `audio_service.dart:178-204` ← `Telegram/SourceFiles/media/player/media_player_widget.cpp:58-59` & `:148-154`
...
- [ ] [CRITICAL] QR-expiry refresh calls `startAuth`, which resets the engine flow to the `"choose"` method-picker and emits a `"choose"` event — bouncing the user off the QR screen every time the code expires (~30s) instead of re-exporting the token in place. The guard `result.state == 'qr'` (line 304) can never be true because `startAuth` returns `"choose"`, so the in-body update is dead; but the damage is already done — `startAuth` replaced the engine's QR core and fired a `"choose"` event that `_handleAuthEvent` applies to `_currentAuth`. AyuGram's `QrWidget::refreshCode` re-exports the login token and stays on the QR step, rescheduling `_refreshTimer` at expiry. The correct engine path here is `submitAuthInput` → `advanceAuth` case `AuthStateQR` (`go/engine/auth.go:415-441`, which calls `RefreshQRToken` and returns a fresh `qr`/`ready` state) — that path is NEVER called from Dart. — `auth_state.dart:303` ← `AyuGram/Telegram/SourceFiles/intro/intro_qr.cpp:417`
- [ ] [CRITICAL] QR-expiry refresh guard is unreachable: `if (result != null && result.state == 'qr')` after a `startAuth` that returns `"choose"` — so a fresh QR is never installed and `_updateQrExpiryTimer` is never re-armed; the QR code silently stops refreshing while the engine has already reset the flow. AyuGram reschedules the refresh on every token result via `_refreshTimer.callOnce(max(left,1)*1000)`. — `auth_state.dart:304` ← `AyuGram/Telegram/SourceFiles/intro/intro_qr.cpp:441`
- [ ] [CRITICAL] SRP_ID_INVALID "transparent retry" block is dead code: it gates on the exact-equality `rawError == 'SRP_ID_INVALID'`, but the engine surfaces a WRAPPED error string (`SubmitAuthInput` sets `Message = err.Error()` → `"auth: user auth: …: SRP_ID_INVALID"`, `go/engine/auth.go:126-135`; `Submit2FA` wraps via `fmt.Errorf("%w: %s", ErrAuth, err)`, `go/cores/telegram.go:1235`). The whole retry/storm-detection branch (lines 121-164) is therefore never entered; execution falls to `else { _error = rawError }` (line 166). The codebase's own convention elsewhere uses substring match — `auth_screen.dart:802` correctly does `raw.contains('SRP_ID_INVALID')`. AyuGram detects the bare type `u"SRP_ID_INVALID"_q` and routes to `handleSrpIdInvalid()`. — `auth_state.dart:121` ← `AyuGram/Telegram/SourceFiles/intro/intro_password_check.cpp:157`
- [ ] [CRITICAL] Even if the SRP branch were reached, the retry is broken: it calls `_engine.startAuth(auth.accountId)` (line 142) to "re-fetch fresh SRP params," but `startAuth` discards the auth flow (fresh core, empty `collected` — losing phone + OTP) and returns `"choose"`, not `"2fa"`. So the re-submit guard `_currentAuth?.state == '2fa'` (line 151) is never true and the auto-resubmit `await submitInput(retryPassword)` (line 157) is unreachable; the user is dumped at the method picker mid-password-entry. AyuGram's `handleSrpIdInvalid → requestPasswordData → passwordChecked` re-fetches ONLY `MTPaccount_GetPassword` and re-runs `MTPauth_CheckPassword` with the retained hash, staying on the password step with zero UI disruption. The correct retry already exists in the core (`go/cores/telegram.go:1059-1071` re-fetches `AccountGetPassword` and re-checks once) — the Dart block is a broken, redundant duplicate that should not exist. — `auth_state.dart:142` ← `AyuGram/Telegram/SourceFiles/intro/intro_password_check.cpp:177`
- [ ] [MAJOR] The retained `_last2faPassword` / `_lastSrpIdInvalidTime` storm machinery (lines 33-36, 121-179) is built entirely on the unreachable branch above, so the storm-abort path that mirrors `handleSrpIdInvalid`'s "second hit within 60s → ServerError" (`intro_password_check.cpp:171-174`) never executes. On a real double-SRP_ID_INVALID the engine error is surfaced verbatim and `auth_screen.dart:802` shows "Session expired, retrying…" while NOTHING retries — false feedback to the user. — `auth_state.dart:123` ← `AyuGram/Telegram/SourceFiles/intro/intro_password_check.cpp:171`
...
- [ ] [MAJOR] `_handleMsgReceived` inserts a live incoming message into the message list whenever the chat is active and not in scheduled/edit-history view — but it does NOT exclude the **deleted-messages view**. The gate is `isActiveChat && !_isScheduledView && !_isEditHistoryView` (missing `&& !_isDeletedMessagesView`), so a newly-arrived message gets `_messages.insert(0, …)` into the deleted-messages list, polluting a read-only view. `_handleMsgEdited` correctly guards both edit AND deleted views (chat_state.dart:2732-2733), proving the omission is a bug. AyuGram's deleted/edited history is a separate read-only section sourced solely from local storage (`AyuMessages::getDeletedMessages`), never from live message events. — `chat_state.dart:2575` ← `AyuGram/Telegram/SourceFiles/ayu/ui/message_history/history_inner.cpp:741`

- [ ] [MAJOR] Returning to the normal message view from scheduled/edit/deleted views does not reset `_hasMoreMessages = true`, so upward pagination silently breaks. `toggleScheduledView` off-branch (chat_state.dart:527), `closeEditHistory` (chat_state.dart:575) and `closeDeletedMessages` (chat_state.dart:597) all call `_loadMessages()` after the special view left `_hasMoreMessages=false` (set at lines 546/669/619), and `_loadMessages` only ever sets the flag to `false`, never back to `true`. Result: after toggling scheduled/edit/deleted and back, `loadMoreMessages` early-returns on `!_hasMoreMessages` and older history can never load. `returnToLatest` does it correctly (`_hasMoreMessages = true` at chat_state.dart:1801), confirming the omission. AyuGram resets the history load state on every re-init (`_historyInited = false`) and recomputes from the data-layer slice rather than carrying a stale "no more" flag across sections. — `chat_state.dart:527` ← `AyuGram/Telegram/SourceFiles/history/history_widget.cpp:2876`

- [ ] [MAJOR] `navigateChatHistory` (back/forward / Ctrl-Tab switch) re-activates a chat with a partial copy of `openChat`'s setup. It sets `_activeChat`/loads messages but omits `onChatActivated?.call()` (so the target chat's on-screen notifications are NOT dismissed), `_startPolling()`, `_activeChannelId = null`, `_hiddenKeyboardMsgId = null`, and the `_isViewingSavedSublists`/forum reset — all of which `openChat` performs (chat_state.dart:1137-1190). In AyuGram, history navigation routes through `chatEntryHistoryMove → jumpToChatListEntry → showThread`, i.e. the identical full activation path used for a normal open, so notifications clear and section state resets the same way every time. — `chat_state.dart:1064` ← `AyuGram/Telegram/SourceFiles/window/window_session_controller.cpp:2291`

- [ ] [MAJOR] `addChatToFolder` is fire-and-forget: it calls `_engine.addChatToFolder(...)` but never reloads `_folders` nor calls `notifyListeners()`, so the local `FolderInfo.chatIds` stays stale and `chatsForFolder` won't include the newly-added chat until an unrelated reload occurs. Sibling mutations refresh correctly — `editFolder`/`removeChatFromAllFolders` await then `loadFoldersForAccount` (chat_state.dart:909/940). AyuGram's `ChatFilters::set`/`reload` refresh the filter set after any change so the dialog list reflects it immediately. — `chat_state.dart:2193` ← `AyuGram/Telegram/SourceFiles/data/data_chat_filters.cpp:457`

- [ ] [MAJOR] `removeSavedReactionTag` is fire-and-forget: it calls `_engine.removeSavedReactionTag(...)` but never reloads `_savedReactionTags` nor notifies, leaving the removed tag visible in the tag filter bar. Its sibling `renameSavedReactionTag` awaits then calls `loadSavedReactionTags()` (chat_state.dart:1556), confirming the missing refresh. AyuGram calls `Reactions::refreshMyTags()` after every saved-tag mutation so the tag list re-syncs. — `chat_state.dart:2197` ← `AyuGram/Telegram/SourceFiles/data/data_message_reactions.cpp:445`
...
- [ ] [MAJOR] Color reference resolution diverges from the C++ semantics. Dart resolves `name: ref;` with an **order-independent fixpoint** and, when `ref` is a palette key not explicitly set in the file, substitutes that key's **day-blue default** (`fallbackMap[ref]`). AyuGram resolves references **strictly in-order, single-pass**: `setColor(name, from)` returns `ValueNotFound` unless `from` was already explicitly `Loaded` earlier in the same file — a forward reference, or a reference to a key the theme doesn't define, is **skipped**, leaving `name` to take *its own* default at `finalize()`. Result: for such themes the Dart assigns `name = ref`'s default while AyuGram assigns `name = name`'s default (and forward refs resolve in Dart but not in AyuGram) → wrong palette colors. — `theme_file.dart:143-156` (esp. fallback branch `151-154`, order-independent loop `143-145`) ← `AyuGram/Telegram/lib_ui/ui/style/style_core_palette.cpp:130-131` (+ in-order driver `AyuGram/Telegram/SourceFiles/window/themes/window_theme.cpp:218-232`, skip path `206-208`)

- [ ] [MAJOR] Malformed-theme handling diverges: AyuGram **hard-rejects the entire theme** on any syntax error — a missing trailing `;`, a missing `:`, or an empty name/value makes `readNameAndValue` return false, which propagates through `ReadPaletteValues`→`loadColorScheme`→`LoadTheme` so the theme is refused and the previous theme stays (user sees a "Theme Error"). Dart's `parsePaletteText` instead **silently skips** any bad line (no colon → `continue`; unparseable hex → color dropped) and treats the trailing `;` as **optional**, so it loads a partial theme with the offending colors falling back to defaults. Different error/empty-state behavior for corrupt themes (UniClient renders a half-applied palette where AyuGram declines). — `theme_file.dart:112-131` (skip-on-error at `115`,`124`,`128`; optional `;` at `123`) ← `AyuGram/Telegram/SourceFiles/window/themes/window_theme.cpp:158-160` (mandatory `;` → return false) + `1525-1528` (`ReadPaletteValues` aborts whole parse)
...
- [ ] [MAJOR] Chat history wallpaper/background is not rendered — the chat reading area is filled with a flat `windowBg`, but AyuGram paints the default chat wallpaper (loads `background.tgv`, applies `DefaultWallPaper().backgroundColors()` gradient + pattern opacity) over the `_history` region on top of the base `windowBg` fill. For light themes especially, this makes the chat area show a flat white/themed color instead of the patterned wallpaper bubbles sit on — a large-area visual deviation. — `theme_preview.dart:295` (windowBg fill, no wallpaper drawn in `_drawMessageArea` 334-399) ← `window/themes/window_theme_preview.cpp:445` (`paintHistoryBackground()` call) + `window/themes/window_theme_preview.cpp:462`

- [ ] [MAJOR] Audio-message waveform "played" coloring is wrong — Dart colors a bar active (`msgWaveformOutActive`) when the BAR index `i < waveActive(33)`, but `waveActive=33` indexes the 67-element waveform DATA array, not the ~35 downsampled bars. Result: ~33/35 ≈ 94% of bars render as "played" (active color), whereas AyuGram compares the DATA index (`if (i >= bubble.waveactive)`, `i` iterating the 67 samples) so only ~half the bars are active. The preview's voice message therefore looks ~fully played instead of half-played. (Secondary: bar pitch uses `barSpacing=2` vs `msgWaveformSkip=1px`, so 4px vs 3px pitch.) — `theme_preview.dart:610` (`i < waveActive`) ← `window/themes/window_theme_preview.cpp:951` (waveform bar loop, `i >= bubble.waveactive`)
