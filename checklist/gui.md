...
- [ ] [MAJOR] The Calls-settings "Use peer-to-peer with" picker (Everyone / My contacts / Nobody) is dead-wired by the same vocabulary-mismatch class (found while verifying the audio-device item above). `_CallsSettingsTab` uses the privacy key `'phone_p2p'` (read `settings_screen.dart:2364`, write via `_setP2P` `:2399`) but the engine's `privacyKeyMap` (`go/bridge/dispatch_engine.go:6959-6973`) has no such key — the P2P call-privacy key is `'calls_p2p'` (→ `InputPrivacyKeyPhoneP2P`). Live logs: `GetPrivacySetting FAILED: unknown privacy key: phone_p2p`. Effect: the read fails so the radio always shows the `?? 'contacts'` fallback regardless of the server value, and `setPrivacySetting(..,'phone_p2p',..)` likewise errors so a selection never reaches Telegram (swallowed). Fix: change `'phone_p2p'` → `'calls_p2p'` in both the read and `_setP2P`. — `settings_screen.dart:2364,2399` ↔ `go/bridge/dispatch_engine.go:6959`
