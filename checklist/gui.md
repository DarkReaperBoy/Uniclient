...
## ayu_filters_page — Filter entity extraction throws FormatException on non-JSON contentRich (caught, plain-text fallback)

- [ ] [MINOR] [DISCOVERED 2026-05-30 Stage-2 testing] `_extractSingleText` (`ayu_filter.dart:192`) decodes `msg.contentRich` as a JSON entity `List` to pull `url`/`text_url` targets for regex matching. For messages whose `contentRich` is non-empty but not a JSON array it throws `FormatException: Unexpected character (at character 1)`, caught at `ayu_filter.dart:214-216` and logged as `FILTER: entity extraction failed for msg <id>` (observed 19× while a shared filter was active). Impact: URL-entity-based filtering silently skips those messages — visible-text matching still works, non-fatal, no crash. Needs a check of what `chat_state` writes to `contentRich` (it appears to sometimes hold plain text rather than the expected entities-JSON). — `ayu_filter.dart:186-217`
  - **[STAGE-1 2026-05-30: ROOT CAUSE FIXED — pending Stage-2 verify]** Not plain text — **base64**. Root cause is in the data source, not the filter: `CachedMessage.fromJson` (`engine_models.dart:848`) base64-decoded `content_raw` (a Go `[]byte`) via `_decodeContentRawTop` but left `content_rich` (also `[]byte`) raw via `safeStr`. Go's `json.Marshal` base64-encodes `[]byte`, so messages arriving over the JSON event stream (`msg_received` → `MsgReceivedEvent.fromJson`, `engine_service.dart:6097`) carried `contentRich` as base64 (e.g. `W3si…` = base64 of `[{"…`); `jsonDecode(...) as List` then threw `Unexpected character (at character 1)`. The proto/RPC path (`engine_service.dart:6300`) already `utf8.decode`s the bytes, which is exactly why ONLY event-delivered (newly-arrived) messages failed (the 19×). **FIX:** added `_decodeContentRich` (base64→utf8; a non-base64 value such as an already-decoded `[…]` array starts with `[` which is not base64-valid, so it falls through unchanged via the catch) and used it at `engine_models.dart:848`. This repairs ALL `content_rich` consumers at once for event-delivered messages: filter URL/text_url extraction (`ayu_filter.dart`), notification spoiler masking (`_applySpoilerEntities`, `chat_state.dart:2471`), notification body entities (`_composeBodyEntities`, `notification_types.dart:425`), custom-emoji preload (`_preloadCustomEmoji`, `chat_state.dart:2164`), bubble entity render (`message_bubble.dart:5949`). Also aligned `_extractSingleText`'s catch (`ayu_filter.dart:214`) to the 4 sibling consumers' silent `catch (_) {}` (was the lone `debugPrint`, the source of the 19× log spam). VERIFIED: clean `flutter analyze` (no new issues; one pre-existing unrelated `prefer_const_declarations` info), builds, launches & renders desktop chat list; zero `FormatException` / `FILTER: entity extraction` in logs while live `msg_received` events processed.

# ayu_general_page — Property name mismatches with AyuGram Desktop

**[STAGE-1 2026-05-30 — all 6 items FALSE POSITIVE (already aligned) — pending Stage-2 verify]**
All 6 "property name mismatch" findings are stale: the Dart code already uses AyuGram Desktop's exact method names. The OLD names the audit cites (`hideSimilarChannelsTab`, `increaseContentHeight`, `increaseContentWidth`, `confirmStickers`, `confirmGifs`, `confirmVoiceMessages`) appear ONLY in this checklist — a whole-repo grep finds ZERO references in any `.dart`/`.go` file. The rename was completed in prior work that didn't update this section. Each name was verified byte-exact against AyuGram ground-truth `ayu/ayu_settings.h` (read directly). All 6 are fully wired in `app_state.dart` (private field + getter + functional setter with `notifyListeners()`+`_saveWindowPrefs()` + reset-default + load-from-prefs + save-to-prefs). NO code change needed. RUNTIME VERIFIED: built debug, launched (existing Telegram session), navigated Settings → AyuGram Preferences → General at desktop 1024×768; all 6 settings render under their sections (Disable Similar Channels collapsible with "Hide Similar Channels Tab" nested check; Webview "Bigger Window" collapsible with Increase Content Height/Width; Confirmations For Stickers/GIFs/Voice Messages) and live-toggling the "For Stickers" row flipped its switch green then back to off (setter fires end-to-end). No crash, no FormatException; only unrelated pre-existing log noise (tray `MissingPluginException`; transient `Scrollbar ScrollController has no ScrollPosition` on settings nav).
...
- [ ] [MAJOR] Property name `hideSimilarChannelsTab` doesn't match AyuGram's `hideSimilarChannels()` — `ayu_general_page.dart:78` ← `AyuGramDesktop/ayu/ayu_settings.h:283` (method exists as `hideSimilarChannels()`, NOT `hideSimilarChannelsTab()`)
  - **[STAGE-1 2026-05-30: FALSE POSITIVE — code already uses `hideSimilarChannels`]** `app_state.dart`: field `_hideSimilarChannels` (L528), getter (L908), setter `setHideSimilarChannels` (L1906-1911), reset (L1683), load `'hideSimilarChannels'` (L3774), save (L4032); UI `ayu_general_page.dart:64,77-78`. Byte-exact match to AyuGram `hideSimilarChannels()` (`ayu_settings.h:283`). `hideSimilarChannelsTab` = ZERO code refs. NO change needed.
- [ ] [MAJOR] Property name `increaseContentHeight` should be `increaseWebviewHeight` to match AyuGram — `ayu_general_page.dart:158` ← `AyuGramDesktop/ayu/ayu_settings.h:288` (C++ uses `increaseWebviewHeight()`)
  - **[STAGE-1 2026-05-30: FALSE POSITIVE — code already uses `increaseWebviewHeight`]** `app_state.dart`: field `_increaseWebviewHeight` (L534), getter (L914), setter `setIncreaseWebviewHeight` (L1942-1947), reset (L1689), load `'increaseWebviewHeight'` (L3780), save (L4038); UI `ayu_general_page.dart:148,157-158`. Byte-exact match to AyuGram `increaseWebviewHeight()` (`ayu_settings.h:288`). `increaseContentHeight` = ZERO code refs. NO change needed.
- [ ] [MAJOR] Property name `increaseContentWidth` should be `increaseWebviewWidth` to match AyuGram — `ayu_general_page.dart:163` ← `AyuGramDesktop/ayu/ayu_settings.h:289` (C++ uses `increaseWebviewWidth()`)
  - **[STAGE-1 2026-05-30: FALSE POSITIVE — code already uses `increaseWebviewWidth`]** `app_state.dart`: field `_increaseWebviewWidth` (L535), getter (L915), setter `setIncreaseWebviewWidth` (L1948-1953), reset (L1690), load `'increaseWebviewWidth'` (L3781), save (L4039); UI `ayu_general_page.dart:149,162-163`. Byte-exact match to AyuGram `increaseWebviewWidth()` (`ayu_settings.h:289`). `increaseContentWidth` = ZERO code refs. NO change needed.
- [ ] [MAJOR] Property name `confirmStickers` should be `stickerConfirmation` to match AyuGram — `ayu_general_page.dart:176` ← `AyuGramDesktop/ayu/ayu_settings.h:344` (C++ uses `stickerConfirmation()`)
  - **[STAGE-1 2026-05-30: FALSE POSITIVE — code already uses `stickerConfirmation`]** `app_state.dart`: field `_stickerConfirmation` (L536), getter (L916), setter `setStickerConfirmation` (L1954-1959), reset (L1691), load `'stickerConfirmation'` (L3782), save (L4040); UI `ayu_general_page.dart:175-176`. Byte-exact match to AyuGram `stickerConfirmation()` (`ayu_settings.h:344`). `confirmStickers` = ZERO code refs. Live-toggled at runtime (For Stickers switch flips). NO change needed.
- [ ] [MAJOR] Property name `confirmGifs` should be `gifConfirmation` to match AyuGram — `ayu_general_page.dart:183` ← `AyuGramDesktop/ayu/ayu_settings.h:345` (C++ uses `gifConfirmation()`)
  - **[STAGE-1 2026-05-30: FALSE POSITIVE — code already uses `gifConfirmation`]** `app_state.dart`: field `_gifConfirmation` (L537), getter (L917), setter `setGifConfirmation` (L1960-1965), reset (L1692), load `'gifConfirmation'` (L3783), save (L4041); UI `ayu_general_page.dart:182-183`. Byte-exact match to AyuGram `gifConfirmation()` (`ayu_settings.h:345`). `confirmGifs` = ZERO code refs. NO change needed.
- [ ] [MAJOR] Property name `confirmVoiceMessages` should be `voiceConfirmation` to match AyuGram — `ayu_general_page.dart:190` ← `AyuGramDesktop/ayu/ayu_settings.h:346` (C++ uses `voiceConfirmation()`)
  - **[STAGE-1 2026-05-30: FALSE POSITIVE — code already uses `voiceConfirmation`]** `app_state.dart`: field `_voiceConfirmation` (L538), getter (L918), setter `setVoiceConfirmation` (L1966-1971), reset (L1693), load `'voiceConfirmation'` (L3784), save (L4042); UI `ayu_general_page.dart:189-190`. Byte-exact match to AyuGram `voiceConfirmation()` (`ayu_settings.h:346`). `confirmVoiceMessages` = ZERO code refs. NO change needed.
...
> **STAGE-1 2026-05-30 correction:** The above Summary/Note describe the ORIGINAL audit state and are now stale. The property names are ALREADY aligned to AyuGram (all 6 verified byte-exact against `ayu_settings.h`); the old Dart-specific names no longer exist in the code. See the STAGE-1 banner at the top of this section. Nothing to align — these are false positives awaiting Stage-2 close.

# ayugram_settings_screen — Layout and visual accuracy issues

**[STAGE-1 2026-05-30 — all 5 items ALREADY IMPLEMENTED in commit `33c4d291` — pending Stage-2 verify]**
All 5 layout/icon/header findings were fixed by a prior Stage-1 session (commit `33c4d291` "[unverified] ayugram_settings_screen: 5 items — match AyuGram layout/icons/header font"; git-blame confirms lines 126/130/136/149/169/319/323-327 all originate there) but that session never annotated this checklist, so the section still read as unaddressed. This session re-verified the committed code 1:1 against AyuGram ground truth (`settings/settings.style:13-16`, `lib_ui/ui/layers/layers.style:148-155`, `ayu/ui/settings/settings_main.cpp:101-132`, plus rendered the three flagged menu PNGs `menu/{all_media,group_reactions,favorite}.png` to confirm their shapes) AND visually at runtime: built debug, launched existing Telegram session, navigated Settings → AyuGram Preferences at desktop 1067px + mobile 400px. All 5 render correctly — icons left-edge at 20px, labels at 60px, "Categories"/"Links" headers semibold, ~9px header→row gap, heart/grid/star icons — no crash, no overflow on the screen. Per-item evidence below.
...
- [ ] [MAJOR] `_FlatCategoryButton` text starts at 79px instead of AyuGram's 60px — outer padding is `fromLTRB(22,10,22,10)` plus a `SizedBox(width:57)` totals 79px for text start; AyuGram `settingsButton` has `padding: margins(60px, 10px, 22px, 10px)` with `iconLeft: 20px` — `ayugram_settings_screen.dart:312-318` ← `AyuGram/SourceFiles/settings/settings.style:13-16`
  - **[STAGE-1 2026-05-30: ALREADY FIXED in `33c4d291` — verified 1:1]** `ayugram_settings_screen.dart:319` is now `EdgeInsets.fromLTRB(20, 10, 22, 10)` with the icon in a `SizedBox(width:40)` (L322-327). Outer left=20 + 40px box ⇒ label text starts at **60px** = `settingsButton.padding.left` (`settings.style:15` `margins(60px,10px,22px,10px)`). The cited 79px / `SizedBox(width:57)` no longer exist. Runtime-confirmed: labels align at ~60px in both desktop & mobile screenshots.
...
- [ ] [MAJOR] `_FlatCategoryButton` icon mispositioned — icon is centered inside a 57px SizedBox starting at 22px left, putting it at ~50px centre; AyuGram uses `iconLeft: 20px` (icon left-edge at 20px, 24px wide, centre ~32px) — `ayugram_settings_screen.dart:316-319` ← `AyuGram/SourceFiles/settings/settings.style:16`
  - **[STAGE-1 2026-05-30: ALREADY FIXED in `33c4d291` — verified 1:1]** Icon is now `Align(centerLeft, Icon(size:24))` inside the `SizedBox(width:40)` (L322-327) under a left padding of 20px, so the icon's **left edge sits at 20px** (centre ~32px) = `iconLeft: 20px` (`settings.style:16`). The old "centered in 57px box at 22px" geometry is gone. Runtime-confirmed.
...
- [ ] [MAJOR] Section header labels ("Categories", "Links") use `FontWeight.w500` (medium) instead of semibold — AyuGram `defaultSubsectionTitle` uses `font(boxFontSize semibold)` = 14px w600 — `ayugram_settings_screen.dart:129` ← `AyuGram/Telegram/lib_ui/ui/layers/layers.style:149-151`
  - **[STAGE-1 2026-05-30: ALREADY FIXED in `33c4d291` — verified 1:1]** Both headers now use `fontSize: 14, fontWeight: FontWeight.w600` ("Categories" L129-130, "Links" L184-185) = `defaultSubsectionTitle` `font(boxFontSize=14 semibold)` (`layers.style:148-151`). No `w500` remains in the file. Runtime-confirmed: headers render visibly semibold.
...
- [ ] [MAJOR] Section header bottom padding 6px instead of 9px — Dart uses `EdgeInsets.fromLTRB(22, 0, 22, 6)`; AyuGram `defaultSubsectionTitlePadding: margins(22px, 7px, 10px, 9px)` has 9px bottom — `ayugram_settings_screen.dart:126` ← `AyuGram/Telegram/lib_ui/ui/layers/layers.style:155`
  - **[STAGE-1 2026-05-30: ALREADY FIXED in `33c4d291` — verified 1:1]** Both headers now use `EdgeInsets.fromLTRB(22, 0, 22, 9)` ("Categories" L126, "Links" L181) ⇒ **bottom = 9px** = `defaultSubsectionTitlePadding` bottom (`layers.style:155` `margins(22,7,10,9)`). The flagged 6px is gone. Note: the AyuGram 7px TOP is externalized into the preceding `SizedBox(height:7)` (L122/L177) so top stays 0 (no double-count); the 10px RIGHT is visually irrelevant for these short left-aligned labels — neither was flagged. Runtime-confirmed.
...
- [ ] [MAJOR] Wrong icons on three category buttons — General uses `Icons.photo_library_outlined` (should be `menuIconShowAll` = `menu/all_media`), AyuGram category uses `Icons.emoji_emotions_outlined` (should be `menuIconGroupReactions` = `menu/group_reactions`), Other uses `Icons.star_border` (should be `menuIconFave` = `menu/favorite`) — `ayugram_settings_screen.dart:135,147,165` ← `AyuGram/SourceFiles/ayu/ui/settings/settings_main.cpp:106,114,128`
  - **[STAGE-1 2026-05-30: ALREADY FIXED in `33c4d291` — verified 1:1 incl. visual]** Rendered the actual AyuGram PNGs: `menu/all_media`=2×2 rounded-square grid, `menu/group_reactions`=heart outline, `menu/favorite`=5-point star outline. Dart now matches each: General `Icons.grid_view_outlined` (L149), AyuGram `Icons.favorite_border` (L136), Other `Icons.star_outline` (L169) — `settings_main.cpp:116/106/131`. Old `photo_library_outlined`/`emoji_emotions_outlined`/`star_border` gone. Runtime screenshots show heart/grid/star rendering correctly. (Filters=`label_outlined`, Appearance=`palette`, Chats=`chat_bubble` were NOT flagged — left unchanged, in scope discipline.)
