...
- [ ] **AuthScreen QR renders placeholder icon, not real QR** — `dart/lib/ui/auth_screen.dart:158-179` `_buildQR` shows a static `Icons.qr_code_2` inside a 180×180 white box instead of encoding `data.qrData` bytes as a scannable QR. Needs a pure-Dart QR library (e.g. `qr_flutter`) wired up so the Telegram login token is actually scannable by the phone app.
