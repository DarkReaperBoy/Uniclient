...
- [x] [MAJOR] `expires` timestamp missing from `StoryItem` — AyuGram `data_story.h:289` has `_expires` (Unix timestamp when story disappears). Neither Go engine (`telegram.go:16778`) nor Dart model expose it. "N hours remaining" countdown on story viewer is impossible — `engine_models.dart:2851` ← `AyuGram/data/data_story.h:289`

- [x] [MAJOR] `forwards` and `reactions` counts missing from `StoryItem` — AyuGram `StoryViews` struct (`data_story.h:78-80`) contains `reactions`, `forwards`, `views`. Dart model only has `views`; the other two stats (shown in story viewer) are unreachable — `engine_models.dart:2861` ← `AyuGram/data/data_story.h:78`
