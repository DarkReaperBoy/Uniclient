...
- [ ] **NoAccountsScreen RenderFlex overflow on fresh launch** — On a fresh vault with 0 accounts, first render logs `A RenderFlex overflowed by 2831 pixels on the bottom.` The grid renders correctly but a `Column`/`Wrap` inside `Center` isn't height-constrained. `dart/lib/ui/shell.dart:316-349` `_NoAccountsScreen.build`.
