...
- [ ] [CRITICAL] `_dispatch` skips the DefaultManager toast entirely when DND is active (`if (_manager is DefaultManager && dnd) { ... skip }`) — C++ `SkipToastForCustom()` on Linux returns `false` unconditionally, meaning custom popup notifications are NEVER suppressed by DND. Users lose all custom popup notifications whenever DND mode is active. — `notification_system.dart:510-514` ← `notifications_manager_linux.cpp:185-187`

- [ ] [CRITICAL] `forceSilent |= dnd` suppresses sound and flash-bounce for the DefaultManager when DND is active — C++ `MaybePlaySoundForCustom` and `MaybeFlashBounceForCustom` on Linux are pass-throughs (`playSound()` / `flashBounce()` with no inhibit check). DND is intentionally NOT applied to sound/flash for custom notifications on Linux; only the native manager uses `invokeIfNotInhibited`. Dart incorrectly silences custom notifications under DND. — `notification_system.dart:516` ← `notifications_manager_linux.cpp:189-195`

- [ ] [CRITICAL] `spoilerLoginCode` detection uses string-matching on `senderName` against a hardcoded set `{'notifications', 'verifycodes'}` — C++ checks `peer->isNotificationsUser() || peer->isVerifyCodes()`, which are protocol-level peer type flags set by the Telegram API (not derived from display names). The string approach false-positives on any user/group named "notifications" or "verifycodes", and false-negatives when usernames are localised or changed. C++ also gates on `!item->out()`, which the Dart omits (spoiler applied to own outgoing OTP messages when it shouldn't be). — `notification_system.dart:463-485` ← `notifications_manager.cpp:1104-1108`

- [ ] [MAJOR] `hideMarkAsRead` is not set when preview text is disabled — C++ derives `hideMarkAsRead` from `hideMessageText || ...`, where `hideMessageText` is `true` when the "show preview" setting is off or the device is passcode-locked. Dart's `_dispatch` only checks `isChannel || slowmodeActive || requiresStars || isReaction || isPollVote` and never propagates `!settings.previewText → hideMarkAsRead`. Result: the "Mark as Read" action button appears in native notifications even when text preview is turned off. — `notification_system.dart:488-497` ← `notifications_manager.cpp:1093-1096`

- [ ] [MAJOR] `_DndChecker` polls DBus every 30 seconds, leaving DND state stale for up to 30 s — C++'s `invokeIfNotInhibited` in the Linux native manager calls `_interface.get_inhibited()` synchronously on every sound/flash-bounce invocation (live read). The Dart `_DndChecker` timer fires every 30 s and caches the result; the `NativeManager` does re-read `Inhibited` per-call, but `NotificationSystem._dispatch` uses the stale cached value for all DND decisions including toast suppression. — `notification_system.dart:61` ← `notifications_manager_linux.cpp:873-876`
