...
_Closed this section (verified live, both desktop+mobile where applicable): folder-removal checkbox now shown for groups/channels/bots-in-folder with entity-type labels (`remove_channel`/`remove_bot` confirmed live, `remove_group` is the else-branch); paid-post detection→persistence→delete-site wiring landed (migration V41, no scan regressions, GetMessages/GetPinnedMessages OK); SearchChats SQL fixed (46 cols match `scanChats`) — global search renders in desktop+mobile._

- [ ] [MINOR] Info-page (Channel/Group Info) "Leave Channel"/"Leave Group" button uses a plain `AlertDialog` (`info_panel.dart` `_confirmLeave`, lines 4561/4880) with no "Remove … from all folders" checkbox. AyuGram routes the Info-page leave through `Window::DeleteAndLeaveHandler` → `DeleteChatBox` — the SAME box (`moderate_messages_box.cpp:949`) that carries `maybeChatsFiltersCheckbox` — so a group/channel in a folder should offer the checkbox there too. The chat-list context-menu + chat-view top-bar leave paths were correctly fixed this section (they call `showDeleteConfirmBox` with `isInFolder`); only this Info-page path still lacks it. ← `info/profile/info_profile_actions.cpp:2978` → `window/window_peer_menu.cpp:3901` → `boxes/moderate_messages_box.cpp:949,1045`
