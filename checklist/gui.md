...
- [ ] spec &sect;33.5 "Add Contact Dialog": The "Add Contact" button calls `showAddContactBox` from `input_dialogs.dart`, which opens `_AddContactBoxContent` — NOT `_AddContactBox` from `contacts_screen.dart`. The `_AddContactBox` class (with the field icon at contactIconPosition (-5, 23)) is dead code. Fix: wire the "Add Contact" button to use `_AddContactBox` from `contacts_screen.dart`, or add the icon to the `input_dialogs.dart` version.
- [ ] spec &sect;33.5 "Country Code Picker": The actual country picker opened is `_CountryPickerContent` from `input_dialogs.dart` (NOT `_CountrySelectBox` from `contacts_screen.dart`). `_CountryPickerContent` is missing: (1) keyboard navigation (↑↓ / PageUp/PageDown / Enter to select), (2) "No countries found" empty state, (3) row height is 40px not spec's 36px, (4) shows flag emoji (spec: text-only rows). The spec-compliant `_CountrySelectBox` in `contacts_screen.dart` is dead code — wire the country picker in the Add Contact flow to use it.
