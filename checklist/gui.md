...
## chat_state — Recent sublists count wrong (6 vs 5)

- [ ] [MAJOR] `_kRecentSublistsMax = 6` but AyuGram uses `kShowSublistNamesCount = 5` for the "recent sublists" shown in the Saved Messages header (e.g. sublist name chips). The Dart constant shows one extra slot that never appears in the real app — `chat_state.dart:112` ← `data/data_saved_messages.cpp:35`
...
- [ ] [CRITICAL] Video avatar upload picked via `_pickAndUploadVideo()` only sets `_avatarPath` locally but `_onSave()` calls `editChannelPhoto(path)` which sends it as a still photo — no animated/video chat photo API call (`uploadProfileVideoFile` / `InputChatUploadedPhoto` with `video` flag) is made. The picked video is silently uploaded as a still image, not as an animated profile video — `admin_tools.dart:397-416` ← `edit_peer_info_box.cpp:485` (`savePhoto` stage uses `api_peer_photo.h` video path)

- [ ] [CRITICAL] "Pending Requests" row (line 1069-1083) opens `showMemberListScreen` with `initialTab: _MemberTab.members` — there is no `requests` tab in `_MemberTab`; AyuGram opens a dedicated `RequestsBoxController::Start()` panel. Pending requests are routed to the general members tab instead of a requests-specific screen — `admin_tools.dart:1081` ← `edit_peer_info_box.cpp:1777` (`RequestsBoxController::Start(_navigation, _peer)`)

- [ ] [CRITICAL] Reactions dialog offers only two options ("All Reactions" / "No Reactions") — AyuGram's `EditAllowedReactionsBox` is a full picker letting the admin choose individual emoji reactions and set a count limit per boost level (custom reactions + paid reactions). The simplified binary picker loses the entire per-emoji selection flow — `admin_tools.dart:839-877` ← `edit_peer_reactions.cpp` (full `EditAllowedReactionsBox`)

- [ ] [MAJOR] Bot-specific manage section is entirely absent. When `_isBot == true`, AyuGram shows: Public Link(s), Currency balance, Credits balance, Affiliate Program, Edit Intro, Edit Commands, Edit Settings, Verify Accounts buttons. The Dart file shows the same `_buildAdminControlsSection` for bots as for channels/groups — all bot management buttons are missing — `admin_tools.dart:989-1100` ← `edit_peer_info_box.cpp:1391-1422` (`fillBotUsernamesButton`, `fillBotCurrencyButton`, `fillBotCreditsButton`, `fillBotAffiliateProgram`, `fillBotEditIntroButton`, `fillBotEditCommandsButton`, `fillBotEditSettingsButton`, `fillBotVerifyAccounts`)

- [ ] [MAJOR] "Star Ref" (Affiliate Program) button missing from channel manage section. AyuGram shows it for channels with `canPostMessages` permission. No equivalent exists in the Dart admin controls section — `admin_tools.dart:989-1100` ← `edit_peer_info_box.cpp:1663-1673` (`hasStarRef` / `fillBotAffiliateProgram`)

- [ ] [MAJOR] `showEditPeerTypeBox` (imported from `create_group_wizard.dart`) passes `accountId`/`chatId`/`isChannel` but the type box does not receive or use `_typeDataSavedValue` (privacy pre-state, has-discussion-link flag, current username). AyuGram pre-populates `_typeDataSavedValue` from the peer's current data including `hasDiscussionLink` to gate history visibility refresh. Missing this means the type box opens without the discussion-link hint, which can cause history visibility to show/hide incorrectly after the type box closes — `admin_tools.dart:475-481` ← `edit_peer_info_box.cpp:882-898` (`showEditPeerTypeBox` with `_typeDataSavedValue->hasDiscussionLink` set)

- [ ] [MAJOR] `_showHistoryVisibilityDialog` (lines 689-728) is a custom two-button AlertDialog, but AyuGram's `EditPeerHistoryVisibilityBox` shows a more detailed explanation box. Crucially, the Dart implementation does NOT check `canEditPreHistoryHidden` permission before showing the button or enabling it — the button appears for any megagroup admin, but the API call will fail for admins without that right — `admin_tools.dart:499-507` ← `edit_peer_info_box.cpp:1356-1373` (canEdit permission check before `EditPeerHistoryVisibilityBox`)

- [ ] [MAJOR] Topics lock condition is wrong. Dart checks `chat.memberCount < 200` against a hardcoded value — AyuGram reads this from `appConfig.get<int>("forum_upgrade_participants_min", 200)` (server-configurable). Also, the lock toast message shown when the forum button is tapped while the group has a discussion link is reversed: Dart shows the minimum-member message for the linked-channel case — `admin_tools.dart:464-520` ← `edit_peer_info_box.cpp:104-108,1157-1185`

- [ ] [MAJOR] `_onSave()` does NOT save `noForwards`, `joinToWrite`, or `requestToJoin` changes that were made inside `showEditPeerTypeBox`. AyuGram's save pipeline calls `validateForwards()`, `validateJoinToWrite()`, `validateRequestToJoin()` and then `saveForwards()` / `saveJoinToWrite()` / `saveRequestToJoin()` as dedicated stages. These three settings are lost on save — `admin_tools.dart:1259-1298` ← `edit_peer_info_box.cpp:2083-2260` (validate/save stage queue)

- [ ] [MAJOR] Auto-translate toggle does not check the required boost level from the server. Dart hardcodes `_autoTranslateMinLevel = 3` (line 766) and shows a toast. AyuGram fetches the real required level dynamically via `Data::LevelLimits::channelAutoTranslateLevelMin()` and shows a boost box (not a plain toast) if the level is insufficient — `admin_tools.dart:766-788` ← `edit_peer_info_box.cpp:1221-1268` (`CheckBoostLevel` / `AskBoostBox`)
