...
- [ ] [MAJOR] Peer-level report (chat info → "Report" → pick a reason, e.g. "Other") submits via `engine.reportMessage` with an EMPTY `msgIds` list → server rejects with `MESSAGE_ID_REQUIRED` and the box closes with no user feedback (observed live 2026-06-03). A whole-peer report should route to the peer-report endpoint (`account.reportPeer` / `messages.report` for the peer with no message ids), not the per-message report. NOTE: pre-existing — untouched by commit 3738fe62, separate from the now-fixed report-Lottie item; found during its Stage-2 smoke test. — `confirm_box.dart:2170,2211,2240` (reportMessage calls in `showDynamicReportFlow`)
