...
- [ ] [MINOR] Startup race (seen during Stage-2 verification, pre-existing, not from this section): `_PasscodeLockScreenState._loadSystemUnlockPref` (`main.dart:2754`) calls `EngineService.getPasscodeConfig` before the FFI bridge is initialized → `Bad state: Bridge not initialized. Call init() first.` logged as `__engine.GetPasscodeConfig crashed`. Self-recovers (app continues fine), but the lock screen should await bridge init (or guard `_callRaw`) before issuing the call. ← `bridge/bridge_ffi.dart:97`, `bridge/engine_service.dart:2903`

- [ ] [MINOR] Cold-start render glitch (seen during Stage-2 verification, pre-existing, not from this section): during the first frames — concurrent with the passcode lock screen, ~3s before chats load — the log emits two `ERROR FLUTTER: RenderFlex overflowed` (299999px on the right; 23px on the bottom) plus a `RawTooltipState.build`/`debugCheckHasOverlay` failure (a tooltip built before an `Overlay` ancestor exists). Transient: self-recovers once layout/overlay settle, not visible in steady-state screenshots; exact offending widget not yet pinpointed. Likely the same startup-ordering issue as the bridge-init race above. ← `main.dart` startup / passcode lock screen
