...
- [ ] [MAJOR] Message load sizes are `limit = _isFirstLoad ? 30 : 50` but AyuGram uses `kMessagesFirstPage = 20` / `kMessagesPerPage = 30` — first-load fetches 50% more than needed (30 vs 20) and subsequent pages fetch 67% more (50 vs 30), causing over-fetching and mismatched scroll-anchor behaviour vs desktop — `chat_state.dart:2137` ← `AyuGramDesktop/Telegram/SourceFiles/ayu/ui/message_history/history_inner.cpp:66-68`
  - **[FALSE POSITIVE — left unchanged 2026-05-30, value 30/50 is CORRECT]** `_loadMessages` (`chat_state.dart:2142`) loads the **main chat history** via `getMessages`. The real Telegram Desktop main-history loader is `history/history_widget.cpp:216-217` (`kMessagesPerPageFirst = 30` / `kMessagesPerPage = 50`), used by `firstLoadMessages()`/`loadMessages()`/`loadMessagesDown()` (cpp:4403,4419,4485-4486,4543,4611,4621) — Dart's `_isFirstLoad ? 30 : 50` is a byte-exact match. The cited `ayu/ui/message_history/history_inner.cpp:66-68` is AyuGram's **message edit-history viewer** (paginating a single message's edit revisions, `InnerWidget::requestMore` at cpp:725), NOT the chat message list — wrong reference. Changing 30/50 → 20/30 would BREAK parity with desktop. Do NOT "fix" this.
