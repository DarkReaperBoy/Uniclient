...
- [ ] [MAJOR] Call-settings audio-device enumeration & selection use a device-type vocabulary the engine rejects, so the **general Output/Input device pickers are non-functional** (found while verifying the now-closed `SetCallDevice` item). `_loadSettings` calls `engine.getAudioDevices(id,'playback')` / `'capture'` (settings_screen.dart:2361-2362) and the picker callbacks call `engine.setCallAudioDevice(id,'playback'/'capture',dev)` (settings_screen.dart:2505/2516), but the engine's `GetAudioDevices` (cache_chats.go:2017-2025) and `SetCallAudioDevice` (cache_chats.go:2167-2183) only accept `output`/`input`/`camera` (+`call_output`/`call_input`). Live logs show `GetAudioDevices FAILED: unknown device type: playback`/`capture`, so the lists fall back to `['Default']` and any non-default selection errors (swallowed). Compounding it, `enumerateAudioDevices` (cache_chats.go:2030-2032) is an empty stub returning `[]string{}` — even with correct type strings no real devices ever appear (ZERO-placeholder violation). Fix: align the type strings (`playback→output`, `capture→input`, or add engine-side aliases) AND implement real device enumeration. — `settings_screen.dart:2361,2362,2505,2516` ↔ `go/engine/cache_chats.go:2017,2030`
