...
- [ ] [MAJOR] `startAllHiding()` has no mechanism to start hiding the HideAll button — it only starts dismiss timers for individual items but never calls `onHideAllChanged` or any equivalent callback, so the HideAll button stays fully visible instead of fading when the user's mouse leaves the notification area — `notification_manager_default.dart:150` ← `AyuGramDesktop/Telegram/SourceFiles/window/notifications_manager_default.cpp:202` (`if (_hideAll && _queuedNotifications.size() < 2) { _hideAll->startHiding(); }`)

- [ ] [MAJOR] `stopAllHiding()` has no mechanism to stop the HideAll button from hiding — cancels dismiss timers for individual items only, never calls any callback for the HideAll button, so once the button starts fading it won't stop when the user re-enters a notification — `notification_manager_default.dart:161` ← `AyuGramDesktop/Telegram/SourceFiles/window/notifications_manager_default.cpp:213` (`if (_hideAll) { _hideAll->stopHiding(); }`)

# notification_manager_native — Linux DBus/Portal Notification Manager

- [ ] [MAJOR] Hardcoded English action labels "Open", "Mark as Read", "Reply" instead of localized strings — `notification_manager_native.dart:522-536` ← `platform/linux/notifications_manager_linux.cpp:615-628` (AyuGram uses `tr::lng_open_link`, `tr::lng_context_mark_read`, `tr::lng_notification_reply`; Dart imports `TrStrings` but doesn't use it for these labels)

- [ ] [MAJOR] Placeholder userpic is a solid-color square rectangle, not a circle — `notification_manager_native.dart:249-251` ← `window/notifications_utilities.cpp:27-31` (`img.fill()` fills entire 64×64 image with flat color; AyuGram generates a proper circular userpic via `Ui::EmptyUserpic` and `PeerData::GenerateUserpicImage`; notification badges appear as coloured squares instead of circles)

- [ ] [MAJOR] Missing fallback to app icon when userpic data unavailable — `notification_manager_native.dart:607` ← `platform/linux/notifications_manager_linux.cpp:772-773` (when `forceHideDetails=false` but `rawRgba==null` due to decode failure or missing file, `app-icon` is set to `''` leaving the notification icon-less; AyuGram always falls back to `ApplicationIconName()` when no image hint is present)

- [ ] [MAJOR] `handlesSound` incorrectly gates on DND state, causing app-level sound to bypass DND — `notification_manager_native.dart:158-159` ← `platform/linux/notifications_manager_linux.cpp:873-877` (returns `false` when `_inhibited`, telling callers "native manager does not handle sound" so they play their own audio, defeating DND; AyuGram's `VolumeSupported()` is a static capability check independent of inhibition — inhibition is enforced separately via `invokeIfNotInhibited`)

- [ ] [MAJOR] Userpic PNG filename derived from 32-bit `hashCode` — collision overwrites a different peer's cached avatar — `notification_manager_native.dart:85` ← `window/notifications_utilities.cpp:69` (Dart: `avatarPath.hashCode.toRadixString(16)`; AyuGram: `base::RandomValue<uint64>()` ensuring unique filenames; two distinct avatar paths with colliding 32-bit hashes write to the same file, showing wrong avatar in notifications)

- [ ] [MAJOR] Sound cache grows without bound — temp audio files are never evicted — `notification_manager_native.dart:167-168` ← `platform/linux/notifications_manager_linux.cpp:337` (`_soundCache` copies sound files to `/tmp/uniclient_audio_cache/` and never evicts them short of process exit; AyuGram uses `Media::Audio::LocalDiskCache` which manages cache size and eviction)

# notification_system — NotificationSystem scheduling & dedup

- [ ] [CRITICAL] `clearForAccount` leaves `_pendingTimers`, `_settingWaiters`, `_groupedBuffer`, and `_accountStates` dirty for the removed account — timers already scheduled via `_scheduleDispatch` will still fire after logout and call `_dispatch` with stale account data — `notification_system.dart:643-647` ← `AyuGram/window/notifications_manager.cpp:593-600` (`clearFromSession` calls `clearForThreadIf` which cancels the wait timer and removes all per-thread state atomically; Dart has no equivalent cancellation for these four structures)

- [ ] [CRITICAL] `_settingWaiters.putIfAbsent` at line 266 silently drops every notification after the first for a given chat while mute state is unknown — if N messages arrive during the unknown-state window only the first is dispatched when resolved — `notification_system.dart:264-268` ← `AyuGram/window/notifications_manager.cpp:473-482` (C++ stores the earliest waiter key in `_settingWaiters` but the full notification queue lives in the Thread object; `checkDelayed`→`showNext` walks the entire queue and dispatches all N messages; Dart has no equivalent queue, so messages 2…N are permanently lost)

- [ ] [MAJOR] `_whenMaps` dedup uses thread key `'${data.accountId}:${data.chatId}'` (no topic), while `_lastAlertPerThread` uses `'$accountId:$chatId:$topicRootId'` for topics — after `clearIncomingFromTopic` the alert record is wiped but the dedup entry for that chatId is still live, meaning the next topic notification for a different topic in the same group can be wrongly suppressed — `notification_system.dart:300` vs `notification_system.dart:634` ← `AyuGram/window/notifications_manager.cpp:212-215` (C++ keys both `_whenMaps` and `_whenAlerts` by `not_null<Data::Thread*>`, so each topic is a distinct key and there is no cross-topic collision)

- [ ] [MAJOR] `_activeAccountId` defaults to `''` and the guard `_activeAccountId.isNotEmpty` at line 282 makes the `notifyFromAll=false` filter a no-op until the caller explicitly sets it — notifications from every account are shown even when the setting says only the active account should notify — `notification_system.dart:281-284` ← `AyuGram/window/notifications_manager.cpp:342-345` (C++ always checks `&thread->session().account() != &Core::App().domain().active()` with no "if empty skip" fallback; the active account is always known)
...
## Summary
`ChatState` is fully wired to the engine — no stubs, no fake data, no empty callbacks. All engine calls are real. However, four numeric constants deviate from AyuGram Desktop's ground-truth values, causing incorrect batch sizes in message loading, saved-sublist pagination, and chat navigation history.

---

- [ ] [MAJOR] `_kFirstPerPage = 10` is the first-page sublist count, but AyuGram uses `kListFirstPerPage = 20` — Dart loads half as many sublists on initial open, causing an unnecessary auto-load cascade on every `openSavedSublists()` call — `chat_state.dart:112` ← `AyuGramDesktop/Telegram/SourceFiles/data/data_saved_messages.cpp:33`

- [ ] [MAJOR] `_kPerPage = 50` is the subsequent-page sublist count, but AyuGram uses `kListPerPage = 100` — every "load more" fetch returns half the expected sublists, doubling the number of round-trips needed to populate the full saved-messages list — `chat_state.dart:113` ← `AyuGramDesktop/Telegram/SourceFiles/data/data_saved_messages.cpp:32`

- [ ] [MAJOR] `_maxChatOpenHistory = 30` caps the Ctrl+Tab chat switcher history at 30 entries, but AyuGram uses `kMaxChatEntryHistorySize = 50` — users who navigate many chats will lose older history entries prematurely — `chat_state.dart:124` ← `AyuGramDesktop/Telegram/SourceFiles/window/window_session_controller.cpp:138`

- [ ] [MAJOR] Message load sizes are `limit = _isFirstLoad ? 30 : 50` but AyuGram uses `kMessagesFirstPage = 20` / `kMessagesPerPage = 30` — first-load fetches 50% more than needed (30 vs 20) and subsequent pages fetch 67% more (50 vs 30), causing over-fetching and mismatched scroll-anchor behaviour vs desktop — `chat_state.dart:2137` ← `AyuGramDesktop/Telegram/SourceFiles/ayu/ui/message_history/history_inner.cpp:66-68`
