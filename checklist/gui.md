...
## FOLLOW-UP — gaps found during Stage-2 verification

_Closed this section (verified live, both desktop+mobile where applicable): folder-removal checkbox now shown for groups/channels/bots-in-folder with entity-type labels (`remove_channel`/`remove_bot` confirmed live, `remove_group` is the else-branch); paid-post detection→persistence→delete-site wiring landed (migration V41, no scan regressions, GetMessages/GetPinnedMessages OK); SearchChats SQL fixed (46 cols match `scanChats`) — global search renders in desktop+mobile; Info-page (Channel/Group Info) "Leave" now routes through the same `showDeleteConfirmBox(mode: leaveChat, isInFolder:…)` DeleteChatBox — the "Remove channel from all folders" checkbox renders & toggles live for a channel-in-folder (verified desktop+mobile), and the group path opens the same box with the correct "delete and leave" group body (group-in-folder covered by the shared `_showRemoveFromFoldersCheckbox` gate + byte-identical call site)._
...
- [ ] [MINOR] Cold-start render glitch — PARTIALLY fixed (Stage-2 re-verify 2026-06-01). FIXED: the two `RenderFlex overflowed` (299999px right / 23px bottom) — titlebar `LayoutBuilder` first-frame guard works, 0 occurrences on cold-start; and the `RawTooltipState`/tooltip overlay error — `TelegramTooltip` switch works, 0 occurrences. STILL FAILING: cold-start (passcode lock screen up) STILL emits **2× `ERROR FLUTTER: No Overlay widget found` + a `debugCheckHasOverlay` failure**, reproduced on two consecutive cold starts. Root cause is NOT the tooltip — the stack trace is the passcode **input `TextField`** building its text-selection overlay: `EditableTextState._createSelectionOverlay` → `new TextSelectionOverlay` → `new SelectionOverlay` → `debugCheckHasOverlay` (`editable_text.dart:4283`). The `_PasscodeLockScreen` renders in the `MaterialApp` builder as a sibling of the Navigator, so its `EditableText` has no `Overlay` ancestor (tree: `EditableText → InputDecorator → … → TextField → … → Positioned`). Fix direction: give the lock screen an `Overlay` ancestor (wrap its `Material`/`Stack` body in an `Overlay` with one initial entry, or host the lock screen inside the Navigator/Overlay tree). Still transient/self-recovering, not visible in steady-state screenshots. ← `main.dart` `_PasscodeLockScreen.build` (TextField at ~`main.dart:2969`) / `editable_text.dart:4283`
