...
- [ ] spec §3 "Menu row padding": spec says `margins(61px, 11px, 20px, 9px)`. Code uses `EdgeInsets.only(left: 21, top: 11, right: 20, bottom: 9)` (line 1268). The left padding is 21px (for the icon), and the label starts at 21+24+16=61px which is correct since the spec's 61px is the label's left offset. Matches
- [ ] spec §3 "Menu item order": spec lists: 1) My Profile, 2) Menu Bots, 3) New Group, 4) New Channel, 5) Contacts, 6) Calls, 7) Saved Messages, 8) Settings, 9) Night Mode. Code order in `hamburger_drawer.dart:153-345` is: My Profile, Menu Bots, [divider], New Group, New Channel, Contacts, Calls, Saved Messages, Settings, Night Mode, Ghost Mode, LRead toggle, SRead toggle, Streamer Mode, Archived Chats, System Frame. The divider between My Profile/Bots block and the rest matches spec §3.3. Order is correct
