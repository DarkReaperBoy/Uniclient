...
class _ChatTopBar extends StatelessWidget {
  final ChatInfo chat;
  /// Full pre-aggregated typing text (ChatState.typingUserFor); null when idle.
  ...
  static void _createTopicFromBurger(BuildContext ctx, ChatState chatState, ChatInfo? parentChat) async {
    ...
    final result = await showEditForumTopicBox(ctx,
      isPremium: ctx.read<AppState>().effectivePremium,
...
class _TopBarTypingDots extends StatefulWidget {
  /// Full pre-aggregated typing text (ChatState.typingSummaryFor) — may be
  /// "N people are typing" / "A and B are typing", not just a single name.
  final String userName;
  final Color color;
...
// FormatType, ComposeEntity, RichTextEditingController → compose_entities.dart

class _ChatRichTextEditingController extends RichTextEditingController {
  _ChatRichTextEditingController() : super() {
    customEmojiBuilder = _buildCustomEmoji;
  }

  InlineSpan _buildCustomEmoji(int documentId, String accountId, String altText, int segStart) {
    return WidgetSpan(
      alignment: PlaceholderAlignment.middle,
      child: _ComposeCustomEmoji(
        key: ValueKey('ce_${documentId}_$segStart'),
