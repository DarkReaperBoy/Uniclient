...
| Emoji picker panel | Tabbed emoji/stickers/GIFs, search, recent | PARTIAL (slice 10: emoji panel w/ 9 categories + backspace key + insert-at-caret; slice 50: search field — engine GetEmojiKeywords (Telegram keyword map, fetched lazily per session, en), matches replace the grid w/ 'no emoji found' state; sticker/GIF tabs + recents later) | gui/emoji.go + engine GetEmojiKeywords | P1 |
