...
- PARTIAL: 32 — engine-gated halves (2FA; notifications sound picker; tray; read-receipt inline avatar stack later) or honest scope cuts (avatar corners stay circular; deep-link message permalinks stay on the browser)
...
- CORE-ONLY: 32 — the engine already has the functionality; the GUI just never surfaces it (slice 101: 1:1 call panel; slice 102: group call screen; slice 103: device pickers; slice 115 audit fixed 9 stale rows + slice 116 audit 6 more — join bar, silent bell, report picker, ghost sub-flags, location, contact card were already shipped)
