...
|---|-------|------|--------------------------|-----------|
| B-24 | cores/teamspeak.go `tsExec` `case <-deadline:` (~line 2926) | The 2-second "void command success" path returns `result, nil` even when NO data line was ever received — a silently dropped response (UDP) becomes a FALSE ACK for state-changing commands (`clientmove` joins, sends): the caller sees success while the server may never have executed the command. Every TS3 command is answered by an `error id=` line per protocol, so silence = loss, not success. (The separate dataTimeout path — 500ms after REAL data arrived — is legitimate and stays.) | code: deadline case returns nil with empty `dataLines`; contrast the dataTimeout branch; no live repro needed (loss is structural over UDP) | tests-first: pure-silence → explicit timeout error (ErrNetwork family); keep dataTimeout success; scripted-silent-connection regression test |
