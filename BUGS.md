...
|---|-------|------|--------------------------|-----------|
| B-23 | cores/teamspeak.go:3381-3385 (handshake) | Every connection sends `servernotifyregister event=server/channel/textserver/textchannel/textprivate` and the live server answers all five with `error id=516 msg=invalid\sclient\stype` (ServerQuery-only command) — five wasted round-trips per connection, errors swallowed, and it is unproven whether any notify class actually needs a client-correct registration we never perform. Events for current-channel/text demonstrably arrive regardless. | RX plaintext trace under `UNICLIENT_TS3_DEBUG`: 5× TX → 5× error 516, present in PASSING runs too | research the client-side registration mechanism (or drop the commands); verify no notify class is silently missing; low severity — no observed behavior loss today |
