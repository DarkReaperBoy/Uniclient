...
|---|-------|------|--------------------------|-----------|
| B-26 | cores/teamspeak.go `SendVoice` + voice UI | The client sends voice into channels where the server silently discards it: when `client_talk_power` (20 for guests) < `channel_needed_talk_power`, ts.arcticblaze drops every packet with no error — a user talks into a dead channel believing they're heard (§1.10-adjacent: sound that goes nowhere). Official TS3 clients refuse/lower the mic instead. No talk-power awareness exists anywhere in core or GUI. | live probe table (slice 247): rooms 43 (needed=9999), 47 (9999), 52 (125), 42 (999999) → 0 packets relayed; rooms 70/94 (needed=0) → 75/75 relayed; own power 20 via `clientinfo`; grep for TalkPower in gui/ = none | core tracks needed power per channel (channelinfo on join + property edits) and the voice UI surfaces it before/while sending; the live-test picker already filters (slice 247) |
